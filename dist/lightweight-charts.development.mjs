/*!
 * @license
 * TradingView Lightweight Charts™ v5.0.8-dev+202508222300
...
const barStyleFnMap = {
    ...
    // eslint-disable-next-line @typescript-eslint/naming-convention
    Rectangle: (findBar, rectangleStyle, barIndex, precomputedBars) => {
        return {
            _internal_barColor: rectangleStyle.fillColor,
            _internal_fillColor: rectangleStyle.fillColor,
            _internal_borderColor: rectangleStyle.borderColor,
        };
    },
};
    ...
    _private__recalculatePriceRangeImpl() {
        ...
        if (priceRange !== null) {
            ...
            if (priceRange._internal_minValue() === priceRange._internal_maxValue()) {
                ...
                const minMove = formatterSource === null || this._internal_isPercentage() || this._internal_isIndexedTo100() ? 1 : 1 / formatterSource._internal_base();
...
function isFulfilledRectangleData(data) {
    return data.time1 !== undefined &&
        data.price1 !== undefined &&
        data.time2 !== undefined &&
...
function getRectangleSeriesPlotRow(time, index, item, originalTime) {
    // For rectangles, we use the time1 as the primary time and store rectangle data
    const res = {
        _internal_index: index,
        _internal_time: time,
        _internal_value: [item.price1, Math.max(item.price1, item.price2), Math.min(item.price1, item.price2), item.price2],
        _internal_originalTime: originalTime
    };
    res._internal_time1 = item.time1;
    res._internal_price1 = item.price1;
    res._internal_time2 = item.time2;
    res._internal_price2 = item.price2;
    if (item.fillColor !== undefined) {
        res._internal_fillColor = item.fillColor;
    }
    if (item.borderColor !== undefined) {
        res._internal_borderColor = item.borderColor;
    }
...
function getSeriesPlotRowCreator(seriesType) {
    const seriesPlotRowFnMap = {
        ...
        Rectangle: wrapWhitespaceData(getRectangleSeriesPlotRow),
...
const createPaneView$7 = (series, model, customPaneView) => {
...
const createCustomSeriesDefinition = (paneView) => {
    const definition = {
         ...
         * @internal
         */
        _internal_createPaneView: createPaneView$7,
...
function rectangleData(plotRow) {
    const result = {
        time: plotRow._internal_originalTime,
        time1: plotRow._internal_originalTime, // Default to original time
        price1: plotRow._internal_value[0 /* PlotRowValueIndex.Open */], // Use open as price1
        time2: plotRow._internal_originalTime, // Default to original time
        price2: plotRow._internal_value[3 /* PlotRowValueIndex.Close */], // Use close as price2
    };
    if (plotRow._internal_time1 !== undefined) {
        result.time1 = plotRow._internal_time1;
    }
    if (plotRow._internal_price1 !== undefined) {
        result.price1 = plotRow._internal_price1;
    }
    if (plotRow._internal_time2 !== undefined) {
        result.time2 = plotRow._internal_time2;
    }
    if (plotRow._internal_price2 !== undefined) {
        result.price2 = plotRow._internal_price2;
    }
    if (plotRow._internal_fillColor !== undefined) {
        result.fillColor = plotRow._internal_fillColor;
    }
    if (plotRow._internal_borderColor !== undefined) {
        result.borderColor = plotRow._internal_borderColor;
    }
    if (plotRow._internal_customValues !== undefined) {
        result.customValues = plotRow._internal_customValues;
    }
    return result;
}
...
function checkRectangleItem(rectangleItem) {
    if (!isFulfilledRectangleData(rectangleItem)) {
        return;
    }
    ['price1', 'price2'].forEach((key) => {
        assert(typeof rectangleItem[key] === 'number', `Rectangle series item data value of ${key} must be a number, got=${typeof rectangleItem[key]}, value=${rectangleItem[key]}`);
        assert(isSafeValue(rectangleItem[key]), `Rectangle series item data value of ${key} must be between ${MIN_SAFE_VALUE.toPrecision(16)} and ${MAX_SAFE_VALUE.toPrecision(16)}, got=${typeof rectangleItem[key]}, value=${rectangleItem[key]}`);
    });
}
...
const createPaneView$6 = (series, model) => new SeriesLinePaneView(series, model);
...
const createPaneView$5 = (series, model) => new SeriesBaselinePaneView(series, model);
const createSeries$5 = () => {
    const definition = {
        type: 'Baseline',
        isBuiltIn: true,
...
const baselineSeries = createSeries$5();
...
const createPaneView$4 = (series, model) => new SeriesAreaPaneView(series, model);
...
const areaSeries = createSeries$4();
...
const createPaneView$3 = (series, model) => new SeriesBarsPaneView(series, model);
...
const barSeries = createSeries$3();
...
const createPaneView$2 = (series, model) => new SeriesCandlesticksPaneView(series, model);
...
const candlestickSeries = createSeries$2();
...
const createPaneView$1 = (series, model) => new SeriesHistogramPaneView(series, model);
...
const histogramSeries = createSeries$1();

const rectangleStyleDefaults = {
...
const rectangleSeries = createSeries();
...
class PaneRendererRectangle extends BitmapCoordinatesPaneRenderer {
    constructor() {
        super(...arguments);
        this._private__data = null;
    }
    ...
    _internal_hitTest(x, y) {
        if (!this._private__data) {
            return null;
        }
        for (const item of this._private__data._internal_items) {
            if (item._internal_visible === false) {
                continue;
            }
            const left = Math.min(item._internal_x1, item._internal_x2);
            const right = Math.max(item._internal_x1, item._internal_x2);
            const top = Math.min(item._internal_y1, item._internal_y2);
            const bottom = Math.max(item._internal_y1, item._internal_y2);
            if (x >= left && x <= right && y >= top && y <= bottom) {
                return {
                    _internal_hitTestData: item,
    ...
    _internal__drawImpl({ context: ctx, bitmapSize, horizontalPixelRatio, verticalPixelRatio }) {
        ...
        for (const item of this._private__data._internal_items) {
            ...
            const x1 = Math.round(item._internal_x1 * horizontalPixelRatio);
            const y1 = Math.round(item._internal_y1 * verticalPixelRatio);
            const x2 = Math.round(item._internal_x2 * horizontalPixelRatio);
            const y2 = Math.round(item._internal_y2 * verticalPixelRatio);
            ...
            // Fill the rectangle
            if (item._internal_fillColor) {
                ctx.fillStyle = item._internal_fillColor;
                ctx.fillRect(left, top, width, height);
            }
            // Draw the border
            if (item._internal_borderVisible && item._internal_borderWidth > 0) {
                ctx.strokeStyle = item._internal_borderColor;
                ctx.lineWidth = Math.floor(item._internal_borderWidth * horizontalPixelRatio);
                setLineStyle(ctx, item._internal_borderStyle);
...
class FVGRectangleRenderer {
    constructor() {
        this._private__rectangleRenderer = new PaneRendererRectangle();
    }
    _internal_setData(data) {
        this._private__rectangleRenderer._internal_setData(data);
    }
    draw(target, isHovered = false, hitTestData) {
        this._private__rectangleRenderer._internal_draw(target, isHovered, hitTestData);
    }
    _internal_hitTest(x, y) {
        return this._private__rectangleRenderer._internal_hitTest(x, y);
    }
}
class FVGRectanglePaneView {
    constructor(options) {
        this._private__renderer = new FVGRectangleRenderer();
        this._private__series = null;
        this._private__timeScale = null;
        this._private__rectangleData = [];
        this._private__options = options;
    }
    renderer() {
        // Re-calculate coordinates on every render call for maximum fluidity
        this._private__prepareRendererData(this._private__rectangleData);
        return this._private__renderer;
    }
    _internal_setSeriesAndTimeScale(series, timeScale) {
        this._private__series = series;
        this._private__timeScale = timeScale;
    }
    _internal_setRectangles(rectangles) {
        this._private__rectangleData = rectangles;
        // Don't prepare renderer data here - it will be done on each render call
    }
    _internal_setOptions(options) {
        this._private__options = { ...this._private__options, ...options };
    }
    _private__prepareRendererData(rectangles) {
        if (!this._private__series || !this._private__timeScale) {
            this._private__renderer._internal_setData({ _internal_items: [] });
            return;
        }
        const items = rectangles.map((rect) => {
            // Convert time coordinates with viewport clamping so partially visible rects render
            let x1 = this._private__timeScale.timeToCoordinate(rect.startTime);
            let x2 = this._private__timeScale.timeToCoordinate(rect.endTime);
            const vr = this._private__timeScale.getVisibleRange();
            if (vr) {
                const from = vr.from;
                const to = vr.to;
                const isNum = (v) => typeof v === 'number' && Number.isFinite(v);
                // If start is left of view and end is visible, clamp start to left edge
                if (x1 === null && x2 !== null && isNum(rect.startTime) && isNum(from) && rect.startTime < from) {
                    x1 = 0;
                }
                // If end is right of view and start is visible, clamp end to right edge
                if (x2 === null && x1 !== null && isNum(rect.endTime) && isNum(to) && rect.endTime > to) {
                    x2 = this._private__timeScale.width();
                }
            }
            // Convert price coordinates
            const y1 = this._private__series.priceToCoordinate(rect.upperPrice);
            const y2 = this._private__series.priceToCoordinate(rect.lowerPrice);
            ...
            return {
                ...
                _internal_x2: x2,
                _internal_y2: y2,
                _internal_fillColor: rect.fillColor || this._private__options.fillColor,
                _internal_borderColor: rect.borderColor || this._private__options.borderColor,
                _internal_borderWidth: this._private__options.borderWidth,
                _internal_borderStyle: 0 /* LineStyle.Solid */,
                _internal_borderVisible: this._private__options.borderVisible,
                _internal_visible: true,
                _internal_externalId: rect.id,
            };
        }).filter((item) => item !== null);
        this._private__renderer._internal_setData({ _internal_items: items });
    }
}
class FVGRectanglePrimitive {
    constructor(options) {
        this._private__rectangles = [];
        this._private__timeScale = null;
        this._private__paneView = new FVGRectanglePaneView(options);
    }
    paneViews() {
        return [this._private__paneView];
    }
    updateAllViews() {
        ...
        this._private__paneView._internal_setRectangles(this._private__rectangles);
    }
    attached({ series, chart, requestUpdate }) {
        this._internal_requestUpdate = requestUpdate;
        this._private__timeScale = chart.timeScale();
        this._private__paneView._internal_setSeriesAndTimeScale(series, this._private__timeScale);
    ...
    _internal_setRectangles(rectangles) {
        this._private__rectangles = rectangles;
        this._private__paneView._internal_setRectangles(rectangles);
        if (this._internal_requestUpdate) {
            this._internal_requestUpdate();
        }
    }
    _internal_addRectangle(rectangle) {
        this._private__rectangles.push(rectangle);
        this._private__paneView._internal_setRectangles(this._private__rectangles);
        if (this._internal_requestUpdate) {
            this._internal_requestUpdate();
        }
    }
    _internal_removeRectangle(id) {
        this._private__rectangles = this._private__rectangles.filter(rect => rect.id !== id);
        this._private__paneView._internal_setRectangles(this._private__rectangles);
        if (this._internal_requestUpdate) {
            this._internal_requestUpdate();
        }
    }
    _internal_updateTime(time) {
    ...
    _internal_updateOptions(options) {
        this._private__paneView._internal_setOptions(options);
        if (this._internal_requestUpdate) {
            this._internal_requestUpdate();
        }
    }
...
class FVGRectanglePrimitiveWrapper extends SeriesPrimitiveAdapter {
    setRectangles(rectangles) {
        return this._internal__primitive._internal_setRectangles(rectangles);
    }
    addRectangle(rectangle) {
        return this._internal__primitive._internal_addRectangle(rectangle);
    }
    removeRectangle(id) {
        return this._internal__primitive._internal_removeRectangle(id);
    }
    updateOptions(options) {
        return this._internal__primitive._internal_updateOptions(options);
    }
}
/**
 ...
 * ```
 */
function createFVGRectangles(series, options = {}) {
    const wrapper = new FVGRectanglePrimitiveWrapper(series, new FVGRectanglePrimitive({
        fillColor: 'rgba(38, 166, 154, 0.2)',
        borderColor: 'rgba(38, 166, 154, 1)',
        borderWidth: 1,
        borderVisible: true,
 ...
 */
function version() {
    return "5.0.8-dev+202508222300";
}

export { areaSeries as AreaSeries, barSeries as BarSeries, baselineSeries as BaselineSeries, candlestickSeries as CandlestickSeries, ColorType, CrosshairMode, histogramSeries as HistogramSeries, LastPriceAnimationMode, lineSeries as LineSeries, LineStyle, LineType, MismatchDirection, PriceLineSource, PriceScaleMode, rectangleSeries as RectangleSeries, TickMarkType, TrackingModeExitMode, createChart, createChartEx, createFVGRectangles, createImageWatermark, createOptionsChart, createSeriesMarkers, createTextWatermark, createUpDownMarkers, createYieldCurveChart, customSeriesDefaultOptions, defaultHorzScaleBehavior, isBusinessDay, isUTCTimestamp, version };
